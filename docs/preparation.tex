...
shapes.symbols,chains,shapes.geometric,shapes.arrows,calc,shapes.multipart}
...
\begin{center}
	...
	\hline
	Anzahl 1 & Erste IP & letzte IP & Netzklasse \\ \hline
...
\paragraph{Arbeiten mit externen IT-Systemen}

Der Betrieb muss regeln, wie die Mitarbeiter mit externen IT-Systemen arbeiten 
dürfen. Jeder Mitarbeiter muss über die Gefahren von IT-Systemen Dritter 
aufgeklärt werden. Die Regelungen müssen festlegen, ob und wie schützenswerte
Informationen auf fremden IT-Systemen verarbeitet werden dürfen und wie 
verhindert werden kann, dass Unbefugte die Informationen einsehen können. Wenn
Mitarbeiter mit fremden IT-Systemen arbeiten, muss sichergestellt sein, dass 
alle dabei entstehenden temporären Daten gelöscht werden. 

\subsubsection{Standardanforderungen}

Standardanforderungen sollten erfüllt werden, um die Sicherheit im IT-System zu
gewährleisten. 

\paragraph{Unverzügliche Meldung von Verlusten}

Mitarbeiter sollten den Verlust oder Diebstahl von informationen, IT-Systemen
oder Medien, die verloren gegangen oder gestohlen worden sind, unverzüglich 
ihrem Betrieb melden. Zu diesem Zweck sollte es klare Meldeketten und 
Kontaktstellen innerhalb des Betriebes geben. 

\paragraph{Beseitigung vertraulicher Informationen}

Vertrauliche Infomrationen sollten sicher entsorgt werden. Wenn weggeworfende 
oder defekte Medien und Dokumente sensible Informationen enthalten, müssen die
Datenträger und Dokumente auf ihre eigene Weise entsorgt oder vernichtet 
werden. \\

\paragraph{Rechtlicher Rahmen für mobiles Arbeiten}

Die rechtlichen Rahmenbedingungen für mobiles Arbeiten müssen beachtet und 
geregelt werden. Alle relevanten Punkte müssen entweder durch 
Betriebsvereinbarungen oder durch individuelle Vereinbarungen zwischen dem
Mitarbeiter und dem Arbeitgeber zusätzlich zum Arbeitsvertrag geregelt werden. 

\paragraph{Sicherheitsrichtlinien für mobile Arbeitsplätze}

Alle relevanten Sicherheitsanforderungen für mobile Arbeitsplätze sollten in 
einer Sicherheitsrichtlinie dokumentierte werden, die für mobile Mitarbeiter 
verbindlich ist. Sie sollte auch mit den bestehenden Sicherheitsrichtlinien der 
Einrichtung und mit allen relevanten Abteilungen abgestimmt werden. Die 
Sicherheitsrichtlinie für mobile Arbeitsplätze sollte regelmäßig aktualisiert 
werden. Die Mitarbeiter der Einrichtung sollten im Hinblick auf die aktuelle 
Sicherheitsrichtlinie sensibilisiert und geschult werden. 

\paragraph{Verschlüsselung von tragbaren IT-Systemen und Datenträgern}

Es sollte sichergestellt werden, dass tragbare IT-Systeme und Datenträger 
gemäß den internen Richtlinien gesichert sind. Tragbare IT-Systeme und 
Datenträger sollten verschlüsselt sein. Die kryptographischen Schlüssel sollten 
getrennt vom verschlüsselten Gerät aufbewahrt werden. 

\paragraph{Verwendung eines Bildschirmschutzes}

Wenn IT-Systeme an mobilen Arbeitsplätzen genutzt werden, sollten die 
Mitarbeiter einen Bildschirmschutz für die Bildschirme der IT-Systeme verwenden. 

\subsubsection{Anforderungen bei erhöhter Schutzbedürftigkeit}

Die erhöhte Schutzbedarfsbedürftigkeit wird anhand einer Risikoanalyse 
festgestellt. 

\paragraph{Verwendung von Diebstahlschutzvorrichtungen}

Wenn das eingesetzte IT-System einen Diebstahlschutz bietet, sollte dieser 
genutzt werden. Die Diebstahlsicherungen sollten immer dort eingesetzt werden, 
wo ein erhöhtes Benutzeraufkommen herrscht oder wo es eine hohe 
Benutzerauslastung gibt. Gleichzeitig sollten die Mitarbeiter immer im 
Hinterkopf behalten, dass der Schutz der auf IT-Systemen gespeicherten 
Informationen in der Regel einen höheren Wert hat als die 
Wiederanschaffungskosten des IT-Systems betragen. Die Beschaffungs- und 
Einsatzkriterien für Diebstahlsicherungen sollten an die Prozesse der 
Institution angepasst und dokumentiert werden. 

\paragraph{Verbot der Nutzung unsicherer Umgebungen}

Es sollten kriterien für die Arbeitsumgebung festgelegt werden, die mindestens
erfüllt sein müssen, um sicherzustellen, dass Informationene, die einen erhöhten 
Schutz erfordern, mobil verarbeitet werden dürfen. Die Kriterien sollten 
mindestens folgende Themenbereiche abdecken:

\begin{itemize}
	\item Einsicht und Zugang durch Dritte
	\item abgeschlossene und erforderlichenfalls abschließbare oder bewachte 
		Räume
	\item sichere Kommunikationseinrichtungen und 
	\item eine angemessene Energieversorgung
\end{itemize}
...
Unüberraschenderweise werden um üblichen Dezimalsystem die Ziffern 0 bis 9 
verwendet. 

\subsection{Das Binärsystem}

Das Binärsystem, auch Zweiersystem oder Dualsystem genannt, ist ein 
Zahlensystem, das zur Darstellung von Zahlen nur zwei verschiedene Ziffern 
nutzt. Die Ziffern haben den gleichen Wert wie im Dezimalsystem. 

\subsubsection{Umrechnung in das Dezimalsystem}

Auch das Binärsystem ist - wie das Dezimalsystem - ein Stellenwertsystem. Daher 
kann man von einer gegebenen Binärzahl auf die gleiche Weise den Gesamtwert als
Dezimahlzahl ermitteln. Das heißt, jede Stelle der Zahl hat eine bestimmte 
Wertigkeit. Wenn man die Stellen nun druchnummeriert und bei den Einern mit 0 
beginnt, kann man die Wertigkeit der einzelnen Stellen sehr schön mit der Basis 
2 ausdrücken (1, 2, 4, 8, 16, 32, 64, usw). \\

Beispiel: \\ 

$101 = 2^{2} + 2^{1} + 2^{0} = 5$

\subsection{Das Hexadezimalsystem}

\emph{Hexadezimal} ist ein Mischwort aus dem griechischen hexa (sechs) und dem
lateinischen decem (zehn). Das Hexadezimalsystem ist also ein Stellenwertsystem
zur Basis 16. Weil man 16 Ziffern braucht, werden nicht nur die Ziffern 0 bis 9
verwendet, sondern auch die Buchstaben von A bis F. 

\subsubsection{Umrechnen von Hexa- ins Dezimalsystem}

Man geht vor wie beim Binärsystem, nur dass hier die Basis 16, die Stellenwerte 
also $16^{0}, 16^{1}, 16^{2}, \dots$ sind. \\

Beispiel: \\

$0x5D6A = A \cdot 16^{0} + 6 \cdot 16^{1} + D \cdot 16^{2} + 5 \cdot 16^{3}$

\subsubsection{Umrechnen von Dezimal- ins Hexadezimalsystem}

Auch huer geht man vor wie beim Binärsystem. Man teilt jedoch nicht durch 2, 
sondern durch 16: \\

Hat man eine Dezimalzahl gegeben, geht man nach folgendem Verfahren vor: 

\begin{enumerate}
	\item Teile die Zahl mit Rest durch 16 und notiere den Rest
	\item Teile das Ergebnis wqieder durch 16 und notiere den Rest
	\item Fahre fort, bis dein Ergebnis 0 ist
	\item Die gesuchte Binärzahl sind die Ziffern der Reste, wobai man mit
		dem Rest beginnt
\end{enumerate}

\emph{Achtung: Denke daran, die Reste von 10 bis 15 in die Symbole A bis F 
umzuwandeln}

\section{Programmablaufpläne, Struktogramme und Pseudocode}

\subsection{Beispiel Programmablaufplan/PAP}
...
\subsection{Beipsiel Struktogramm}

\begin{struktogramm}(50,40)
  \ifthenelse{2}{1}{Condition}{ja}{nein}
    \ifthenelse{1}{1}{Condition}{ja}{nein}
      \assign{Stmt}
    \change
      \assign{Stmt}
    \ifend
...
\subsection{Beispiel Pseudocode}
...
\section{Die Grundlagen derObjektorientierten Programmierung}

\subsection{Objekte}

Die OOP dreht sich um Objekte. Ein Objekt ist eine Instanz einer Klasse und hat
Eigenschaften (Attribute) und Methoden. Es wird oft als ein Ding oder eine 
Entität bezeichnet, die bestimmte Eigenschaften hat und bestimmte Aktionen
ausführen kann. 

\subsection{Klassen}

Klassen sind Schablonen, die beschreiben, wie Objekte erstellt werden. Sie sind
eine Sammlung von Eigenschaften und Methoden, die ein Objekt hat. Eine Klasse 
definiert die Attribute und Methoden eines Objekts, aber nicht das Objekt 
selbst. 

\subsection{Vererbung}

Vererbung ist ein Konzept, das es einer Klasse ermöglicht, Eigenschaften und 
Methoden einer anderen Klasse zu erben. Die Klasse, die geerbt wird, wird als 
Elternklasse oder Basisklasse bezeichne6t. Die Klasse, die erbt, wird als 
Kindklasse oder abgeleitete Klasse bezeichnet. 

\subsection{Polymorphie}

Polymorphie bedeutet, dass eine Methode auf verschiedene Arten implementiert 
werden kann. Es gibt zwei Arten von Polymorphie: Überladung (Methoden mit 
demselben Namen, aber verschiedenen Parametertypen) und Überschreibung (die 
Kindklasse definiert eine Methode, die in der Elternklasse definiert ist). 

\subsection{Kapselung}

Kapselung bedeutet, dass die Attribute und Methoden eines Objekts vor externen 
Zugriffen geschützt werden können. Dies wird erreicht, indem die Attribute und 
Methoden als private oder geschützte definiert werden. 
